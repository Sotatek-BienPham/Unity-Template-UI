...
{
    // Singleton
    private static FxCurrency _Instance;
    ...
    public GameObject preXp;
    public GameObject preStamina;
    public GameObject material;

    public SpriteAtlas materialAtlas;
    //[Header("UI")]
    ...
    private static Sprite materialSprite;

    public static FxCurrency Show(Vector3 VectorStart, double value, Vector3 to, Action onFinish = null,
        Action onCollideFx = null, Transform targetTransfom = null, RewardType type = RewardType.Gold,float scale = 1)
    {
        ...
        /* Play Sound Collect Currency Here */
    ...
    public void SetMaterialSprite(string spriteName)
    {
        materialSprite = materialAtlas.GetSprite(spriteName);
    }
    ...
    {
        // if (_type == RewardType.RandomMaterial)
        ...
        while (_count < _max)
        {
            ...
            switch (_type)
            {
                case RewardType.Gold:
                    ...
                    break;
                case RewardType.Gem: // Bur
                    go = LeanPool.Spawn(preGem, _vectorShow, UnityEngine.Quaternion.Euler(0, 0, 0), transform);
                    go.transform.localRotation = UnityEngine.Quaternion.Euler(0, 0, 0);
                    break;
                case RewardType.Exp:
                    go = LeanPool.Spawn(preXp, _vectorShow, UnityEngine.Quaternion.Euler(0, 0, 0), transform);
                    go.transform.localRotation = UnityEngine.Quaternion.Euler(0, 0, 0);
                    break;
                // case RewardType.Stamina:
                //     go = LeanPool.Spawn(preStamina, _vectorShow, UnityEngine.Quaternion.Euler(0, 0, 0), transform);
                //     go.transform.localRotation = UnityEngine.Quaternion.Euler(0, 0, 0);
                //     break;
                // case RewardType.RandomMaterial:
                //     go = LeanPool.Spawn(material, _vectorShow, UnityEngine.Quaternion.Euler(0, 0, 0), transform);
                //     go.transform.localRotation = UnityEngine.Quaternion.Euler(0, 0, 0);
                //     go.GetComponent<Image>().sprite = materialSprite;
                //     break;
                default:
                    go = LeanPool.Spawn(preGold, _vectorShow, UnityEngine.Quaternion.Euler(0, 0, 0), transform);
                    go.transform.localRotation = UnityEngine.Quaternion.Euler(0, 0, 0);
